...
const vscode = require('vscode');
const path = require('path');
const { analyzeComplexity, highlightComplexFunctions } = require('./modules/complexityAnalysis.js');
...
const { generateDashboardHTML } = require('./modules/ui.js');
...
function activate(context) {
    ...
    context.subscriptions.push(
        ...
        vscode.commands.registerTextEditorCommand('extension.generateTestsForAllFunctions', handleBatchTestGeneration.bind(state, null)),
        vscode.commands.registerCommand('extension.showComplexityDashboard', generateDashboardHTML.bind(state, null)),
...
function handleSelectionChange(event) {
    ...
    vscode.window.showInformationMessage("Something was clicked");
    ...
    if (line.lineNumber === document.lineCount - 1 && 
        ...
        if (isBatchClick && event.selections.length > 1) {
            state.lastClickTime = currentTime;
            handleBatchTestGeneration(state,state.activeEditor);
            return;
        }
    }
